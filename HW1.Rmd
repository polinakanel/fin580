...
fx.returns.df$month = months(fx.returns.df$date)
fx.returns.df$year = format(fx.returns.df$date, format="%Y")

fx.returns.daily = aggregate(fx.returns.df[,1:9],by=list(fx.returns.df$date), function(x){ sqrt(var(x)*60*12/5) })
fx.returns.monthly = aggregate(fx.returns.df[,1:9],by=list(fx.returns.df$month, fx.returns.df$year), function(x){ sqrt(var(x)*60*12/5) })
fx.returns.daily$week.id = rep(1:7, times = dim(fx.returns.daily)[1]/7)
